...
async function fetchProposalData(proposalNumber) {
  ...
  const openingTextHtml = richToHtml(proposal['Opening Text'], 'openingText');
  ...
  const formattedLineItems = lineItems.map((item) => {
    ...
    return {
      ...
      details: stripBackticks(extractString(item['Details'])),
...
function richToHtml(value, fieldName) {
  if (!value) return '';
  if (typeof value === 'string') {
    let unwrapped = value.replace(/^```\n?/, '').replace(/\n?```$/, '');
    unwrapped = unwrapped.replace(/##_hidden_grid_##\s*\d*/g, '').trim();
    if (fieldName) {
      console.log(`[debug:${fieldName}] raw (${value.length} chars):\n${value}`);
      console.log(`[debug:${fieldName}] after strip:\n${unwrapped}`);
    }
    return marked.parse(unwrapped);
  }
  if (fieldName) {
    console.log(`[debug:${fieldName}] non-string type=${typeof value}:`, JSON.stringify(value));
  }
  return String(value);
}
