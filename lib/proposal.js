...
async function fetchProposalData(proposalNumber) {
  ...
  const [proposal, lineItems, packages] = await Promise.all([
    coda.getRowByName(TABLES.proposals, { 'Proposal Number': proposalNumber }),
  ...
  const whatsNextHtml = richToHtml(proposal["What's Next"])
    || "<p>If you approve, use the signature section below to indicate your agreement. From there, we'll connect on the details of the deliverables we're working on with you.</p>";
  ...
  const formattedLineItems = lineItems.map((item) => {
    ...
    const billingPeriod = normalizeBillingPeriod(item['Billed']);
    total += cost;
    if (billingPeriod === 'one-time') oneTimeTotal += cost;
    if (billingPeriod === 'monthly') monthlyTotal += cost;
    return {
  ...
  const oneTimeLineItems = formattedLineItems.filter((item) => item.billingPeriod === 'one-time');
  const monthlyLineItems = formattedLineItems.filter((item) => item.billingPeriod === 'monthly');
  const unclassifiedLineItems = formattedLineItems.filter((item) => !item.billingPeriod);
  ...
  return {
    ...
    oneTimeTotal: formatCurrency(oneTimeTotal),
    monthlyTotal: formatCurrency(monthlyTotal),
    specialTerms: specialTermsHtml,
    whatsNext: whatsNextHtml,
    thankYou: thankYouHtml,
    ourSigner: stripBackticks(extractString(proposal['Our Signer'])) || BRANDING.signatory,
    ourSignedDate: formatDateTime(proposal['Our Signed Date']),
...
// Expose templateData as JSON that is safe inside an inline script block.
Handlebars.registerHelper('json', (value) => new Handlebars.SafeString(
...
function normalizeBillingPeriod(value) {
  const normalized = stripBackticks(extractString(value))
    .toLowerCase()
    .replace(/[^a-z]/g, '');
...
module.exports = { fetchProposalData, renderHtml, formatDateTime };
